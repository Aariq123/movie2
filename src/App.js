...
function App() {
  const { menuOpen, closeMenu, openMenu, position, menuName, mobileMenu, setMobileMenu, userData } = useContext(MainContext)
  ...
  const mobileMenuClose = (target) => {
    if (deviceRef) {
      if (deviceRef.current.clientWidth < 767) {
        if (mobileMenu && (target.tagName == 'svg' || target.tagName == 'path' || !target.className.includes('gay'))) {
          setMobileMenu(false)
        }
      }
    }
  ...
  return (
    <div ref={deviceRef} onClick={(e) => mobileMenuClose(e.target)} className="bg-neutral-800 text-white" onMouseOver={(e) => closeMenu(e)}>
      <div ref={ref} className={`gay md:left-0 absolute top-14 bg-black flex flex-col shadow-md ${menuOpen ? '' : 'hidden'} z-30`}>
        {submenu.map(item => {
          return (
      ...
      <nav className="py-4 px-4 sm:py-8 absolute sm:px-12 w-screen flex z-20 justify-between">
        ...
        <div className={`flex bg-neutral-800 flex-col ${mobileMenu ? 'left-0' : 'right-full'} absolute top-0 md:bg-transparent md:static text-lg font-bold w-1/2 md:flex-row justify-between`}>
          <p className="gay border-1 md:border-0 border-black flex justify-between py-4 px-2 md:p-0 cursor-pointer" onPointerOver={(e) => openMenu(e.target.offsetLeft, 'movie')}>Movies<span className={mobileMenu ? 'gay' : 'hidden'}>></span></p>
          <p className="gay border-1 md:border-0 border-black flex justify-between my-2 md:m-0 py-4 px-2 md:p-0 cursor-pointer" onPointerOver={(e) => openMenu(e.target.offsetLeft, 'tv')}>TV-shows<span className={mobileMenu ? 'gay' : 'hidden'}>></span></p>
          <Link className="py-4 px-2 md:p-0" to='/people'>People</Link>
          <Link className="py-4 px-2 md:p-0" to='/account'>{userData ?
            <span className='uppercase text-lg flex flex-col items-center justify-center font-light h-7 w-7 bg-teal-400 rounded-full'>
              {userData && userData.username.split('')[0]}
            </span>
            : <AccountCircleIcon></AccountCircleIcon>}</Link>
        </div>
      </nav>
      <Routes>
      ...
      <footer className="bg-black px-10 py-20 flex justify-evenly">
